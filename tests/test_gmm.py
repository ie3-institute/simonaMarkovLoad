...
def test_sample_value_deterministic(tiny_models, rng):
    ...
    samples = [sample_value(gmms, bucket_idx, state_idx, rng=test_rng) for _ in range(10)]

    test_rng = np.random.default_rng(456)
    samples_repeat = [sample_value(gmms, bucket_idx, state_idx, rng=test_rng) for _ in range(10)]
